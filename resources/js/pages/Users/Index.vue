                        ...
                        <div class="avatar placeholder">
                            <div class="bg-white text-primary rounded-full w-16 h-16 flex items-center justify-center shadow-lg">
                                <span class="text-2xl font-bold">
                                    {{ userDetail ? getUserInitials(userDetail.name) : 'U' }}
                                </span>
                            </div>
                        </div>
                        <div>
...
// View user detail
const viewDetail = (user_id) => {
    isUserDetailModalOpen.value = true;
    userDetail.value = null;
    error.value = null;
    axios.get(`/api/admin/users/${user_id}`).then(response => {
        if (response.data.success) {
            userDetail.value = response.data.data;
        }
        else {
            error.value = response.data.message || 'Failed to load user detail';
        }
    }).catch(err => {
        error.value = 'An error occurred while fetching user detail';
...
// Watch modal state and update dialog element
watch(isUserDetailModalOpen, (newValue) => {
    if (userDetailModal.value) {
        if (newValue) {
